...
class Application(QApplication):
    ...
    def _on_login_successful(self, user_id: int):
        """Handle successful login — recreate session from stored auth.
        ...
        # Re-fetch session from auth service (user just logged in)
        from app.application.services.session import SessionManager, CurrentSession
        from app.infrastructure.database.connection import get_connection
        ...
        # Get user info
        cursor = conn.execute("SELECT * FROM nhan_vien WHERE id = ?", (user_id,))
        row = cursor.fetchone()
        if not row:
            return
        
        # Get vai_tro_ma
        cursor2 = conn.execute("SELECT ma_vai_tro FROM vai_tro WHERE id = ?", (row[6],))
        vt_row = cursor2.fetchone()
        vai_tro_ma = vt_row[0] if vt_row else ""
    ...
    def _on_password_changed(self, user_id: int):
        ...
        if nhan_vien:
            # Get vai_tro_ma from vai_tro table
            cursor = conn.execute(
                "SELECT ma_vai_tro FROM vai_tro WHERE id = ?",
                (nhan_vien.vai_tro_id,)
            )
            row = cursor.fetchone()
            vai_tro_ma = row[0] if row else ""
