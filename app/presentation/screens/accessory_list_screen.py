...
class AccessoryListScreen(QWidget):
    ...
    def _get_selected_id(self) -> int:
        """Get selected accessory ID from table.
        
        Returns:
            Accessory ID or -1 if none selected.
        """
    ...
    def _on_delete_clicked(self):
        ...
        if item_id < 0:
            QMessageBox.warning(self, "Chưa chọn", "Vui lòng chọn phụ kiện cần xoá.")
            return
        
        reply = QMessageBox.question(
            self,
            "Xác nhận xoá",
            "Bạn có chắc muốn xoá phụ kiện này?\n\nHành động này không thể hoàn tác.",
        ...
        try:
            self._pk_service.delete(item_id)
